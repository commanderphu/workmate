...
const formTitle  = ref('')
const formDue    = ref<string | null>(null)      // <input type="datetime-local">
const formNotes  = ref('')
...
function isOverdue(r: Reminder) {
  ...
  const now = Date.now()
  return (r.status ?? 'pending') !== 'done' && due < now
}

function statusLabel(r: Reminder) {
  if ((r.status ?? 'pending') === 'done') return 'done'
  return isOverdue(r) ? 'overdue' : 'pending'
}

function statusClasses(r: Reminder) {
  const s = statusLabel(r)
  if (s === 'done')     return 'bg-emerald-600/25 text-emerald-300'
  if (s === 'overdue')  return 'bg-red-600/25 text-red-300'
  return 'bg-yellow-600/25 text-yellow-300'
}
...
<template>
  <!-- Breiter, zentrierter Wrapper (passt optisch zu deinen anderen Cards) -->
    ...
    <div class="rounded-xl border border-white/5 bg-[#1a1d26] p-6 shadow-lg shadow-black/30 space-y-6">
      ...
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div class="flex items-center gap-3 min-w-0">
          ...
          <input
            ...
            type="text"
            placeholder="Suchen…"
            class="w-[320px] md:w-[420px] max-w-full rounded-lg bg-[#0f121a] text-white placeholder-white/40 border border-white/10 px-3 py-2 outline-none focus:border-white/30"
          />
        </div>

        <button
          class="self-start md:self-auto bg-[#ff9100] hover:bg-[#ffae33] text-black font-semibold rounded-lg px-5 py-2 transition"
          @click="openCreate"
        >
          + Neuer Reminder
        </button>
      ...
      <div v-else>
        ...
        <div
          ...
          class="rounded-lg border border-white/10 bg-white/5 p-8 text-center"
        >
          <div class="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-full bg-white/10 text-white/80">🔔</div>
          <div class="text-white font-medium">Noch keine Reminders</div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-white/10">
              <tr
                v-for="r in filtered"
                :key="r.id"
                :class="['hover:bg-white/5 transition', (r.status ?? 'pending') === 'done' ? 'opacity-60' : '']"
              >
                <!-- Titel + Description + Status-Badge -->
                <td class="px-3 py-2 text-white">
                  <div class="font-medium truncate">{{ r.title }}</div>
                  <div v-if="r.description" class="text-xs text-white/60 truncate">
                    {{ r.description }}
                  </div>
                  <span
                    class="mt-1 inline-block rounded-full px-2 py-0.5 text-[11px] font-medium"
                    :class="statusClasses(r)"
                  >
                    {{ statusLabel(r) }}
                  </span>
                </td>
                ...
                <td class="px-3 py-2">
                  <div class="flex flex-wrap justify-end gap-2">
                    ...
                    <button
                      ...
                      :disabled="(r.status ?? 'pending') === 'done'"
                      @click="markDone(r.id)"
                    >
                      ✅ Erledigt
                    </button>
