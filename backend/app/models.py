...
class Document(Base):
    ...
    document_type: Mapped[Optional[DocumentType]] = mapped_column(
        SAEnum(
            DocumentType,
            name="documenttype",
            create_type=False,  # 🚫 erstellt keinen neuen Typ
            native_enum=True,  # ✅ verwendet Postgres-ENUM
        ),
        nullable=True,
    )
...
class SickLeave(Base):
    ...
    employee = relationship(
        "Employee", back_populates="sick_leaves", passive_deletes=True
    )
...
class VacationRequest(Base):
    ...
    status: Mapped[VacationStatus] = mapped_column(
        SAEnum(
            VacationStatus,
            name="vacationstatus",
            create_type=False,  # 🚫 kein neuer Typ
            native_enum=True,  # ✅ nutzt Postgres ENUM
        ),
    ...
    employee = relationship(
        "Employee", back_populates="vacation_requests", passive_deletes=True
    )
...
class TimeEntry(Base):
    ...
    employee = relationship(
        "Employee", back_populates="time_entries", passive_deletes=True
    )
...
class Reminder(Base):
    ...
    employee = relationship(
        "Employee", back_populates="reminders", passive_deletes=True
    )
